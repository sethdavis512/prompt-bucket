...
import { Link, useOutletContext, useFetcher, useRevalidator } from "react-router"
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  if (!existingPrompt) {
    return { error: "Prompt not found or you don't have permission to edit it" }
  }
  ...
  console.log("Updating prompt with data:", { promptId, userId: session.user.id, title: data.title })
  ...
  try {
    ...
    console.log("Prompt updated successfully:", { id: updatedPrompt.id, title: updatedPrompt.title })
...
export default function PromptDetail({ loaderData }: Route.ComponentProps) {
  ...
  const generatePromptPreview = () => {
    ...
    return promptSections
      .map(section => {
        const content = currentPrompt[section.id as keyof typeof currentPrompt] as string
          ...
          <div className="flex items-start justify-between">
                ...
                <div className="space-y-4 mb-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-4">
                      <label className="flex items-center">
                        <input
                          ...
                          onChange={() => updateEditedValue('public', 'false')}
                      ...
                      <label className="flex items-center">
                        <input
                          type="radio"
                          checked={editedPrompt.public}
                          onChange={() => updateEditedValue('public', 'true')}
                          className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300"
                        />
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      {editedPrompt.public ? 
                        "This prompt can be shared publicly via: /share/" + prompt.id :
                ...
                {prompt.categories?.map((pc: any) => (
                  <span
                    ...
                    style={{
                      ...
                      color: pc.category.color || '#6B7280'
                    }}
                  >
                    {pc.category.name}
                  </span>
            ...
            <div className="flex items-center space-x-2">
              {isEditing ? (
                ...
                <>
                  <button
                    onClick={() => copyToClipboard(buildFullPrompt())}
                    className="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50"
                  >
                    {copiedFull ? (
                      <>
