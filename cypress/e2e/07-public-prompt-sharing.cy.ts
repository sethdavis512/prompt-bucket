    ...
    context('Pro User Sharing Features', () => {
        ...
        it('should create and make prompt public', () => {
            const publicPromptData = {
                ...
                description:
                    'A comprehensive marketing assistant for creating engaging content',
                taskContext:
                    'Act as an expert marketing strategist with 10+ years of experience in digital marketing',
                detailedTaskDescription:
                    'Create compelling marketing copy that converts visitors into customers',
                examples:
                    'Example 1: Email subject lines that increase open rates\\nExample 2: Social media posts that drive engagement'
            };
            ...
            createTestPrompt(publicPromptData).then((createdPrompt) => {
                ...
                cy.get('body').then(($body) => {
                    if (
                        $body.text().includes('View public') ||
                        $body.text().includes('Share')
                    ) {
            ...
            it('should generate shareable public URL', () => {
                ...
                cy.get('[data-cy=taskContext]').type(
            ...
            it('should allow copying prompt content from share page', () => {
                ...
                cy.get('[data-cy=prompt-title]').type(
                    'Copy Test Public Prompt'
                );
                cy.get('[data-cy=taskContext]').type(
                    'This content should be copyable from share page'
                );
                cy.get('[data-cy=detailedTaskDescription]').type(
                    'Detailed instructions that users can copy'
                );
                cy.get('[data-cy=public-checkbox]').check();
                cy.get('[data-cy=save-prompt]').click();
                ...
                cy.url().then((url) => {
                    ...
                    cy.contains('copyable from share page').should(
                        'be.visible'
                    );
                });
            });
            ...
            it('should show prompt metadata on share page', () => {
                ...
                cy.url().then((url) => {
                    ...
                    cy.contains('Comprehensive Public Prompt').should(
                        'be.visible'
                    );
                    cy.contains('detailed prompt showcasing').should(
                        'be.visible'
                    );
                });
            });
        ...
        context('Share Page Access and Security', () => {
            it('should allow unauthenticated access to public prompts', () => {
                ...
                createProUserWithSubscription('sharing-unauthenticated');
                ...
                cy.url().then((url) => {
                    ...
                    cy.contains('Unauthenticated Access Test').should(
                        'be.visible'
                    );
            ...
            it('should return 404 for non-existent share URLs', () => {
                cy.visit('/share/non-existent-prompt-id', {
            ...
            it('should not allow sharing of private prompts', () => {
                createProUserWithSubscription('sharing-private');
        ...
        context('Sharing UI/UX Features', () => {
            beforeEach(() => {
                createProUserWithSubscription('sharing-ui');
            });
            ...
            it('should show sharing status in prompt library', () => {
                ...
                cy.visit('/prompts/new');
                cy.get('[data-cy=prompt-title]').type(
                    'Library Sharing Status Test'
                );
                cy.get('[data-cy=taskContext]').type(
                    'Testing sharing status in library'
                );
                cy.get('[data-cy=public-checkbox]').check();
                cy.get('[data-cy=save-prompt]').click();
                ...
                cy.get('body').then(($body) => {
                    if ($body.text().includes('View public')) {
                        cy.get('[data-cy=prompt-card]').should(
                            'contain',
                            'View public'
                        );
                    }
        ...
        context('Mobile Sharing Experience', () => {
            beforeEach(() => {
                createProUserWithSubscription('sharing-mobile');
            });
            ...
            it('should provide good mobile sharing experience', () => {
                ...
                cy.get('[data-cy=taskContext]').type(
                    'Testing mobile sharing experience'
                );
                cy.get('[data-cy=public-checkbox]').check();
                cy.get('[data-cy=save-prompt]').click();
                ...
                cy.url().then((url) => {
                    ...
                    cy.contains('mobile sharing experience').should(
                        'be.visible'
                    );
