...
export class BasePage {
    ...
    waitForPageReady(timeout = 10000) {
        ...
        cy.get('[data-testid="loading"]', { timeout: 2000 }).should(
            'not.exist'
        );
        return this;
    }
...
export class PromptsPage extends BasePage {
    ...
    waitForPageReady() {
        ...
        cy.contains('Manage and organize your prompt templates').should(
            'be.visible'
        );
        return this;
    }
    ...
    viewPrompt(title: string) {
        cy.get('[data-cy="prompt-card"]')
            .contains(title)
            .parents('[data-cy="prompt-card"]')
            .within(() => {
...
export class PromptCreatePage extends BasePage {
    ...
    fillCompleteForm(promptData: {
        ...
        if (promptData.taskContext)
            this.fillTaskContext(promptData.taskContext);
        if (promptData.toneContext)
            this.fillToneContext(promptData.toneContext);
        if (promptData.detailedTaskDescription)
            this.fillDetailedTask(promptData.detailedTaskDescription);
        if (promptData.examples) this.fillExamples(promptData.examples);
        if (promptData.immediateTask)
            this.fillImmediateTask(promptData.immediateTask);
        if (promptData.outputFormatting)
            this.fillOutputFormatting(promptData.outputFormatting);
    ...
    submitAndVerifySuccess() {
        ...
        cy.url().should(
            'satisfy',
            (url) => url.includes('/prompts/') || url.includes('/dashboard')
        );
        return new PromptDetailPage();
    }
}
